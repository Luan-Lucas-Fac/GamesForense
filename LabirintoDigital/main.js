(() => {
  ...
  const btnCampaign50 = document.getElementById('btnCampaign50');
  ...
  const DIFFS = {
    easy:   { label: 'Fácil',   cols: 24, rows: 16, cell: 32, time: 150, color: '#39ff14', wallColor: '#9dff7a', itemCount: 20, itemValue: 20 }, // 2:30 min
    medium: { label: 'Médio',   cols: 30, rows: 20, cell: 32, time: 210, color: '#faff00', wallColor: '#fff27a', itemCount: 22, itemValue: 22 }, // 3:30 min
    hard:   { label: 'Difícil', cols: 36, rows: 24, cell: 32, time: 300, color: '#ff3131', wallColor: '#ff8a8a', itemCount: 24, itemValue: 24 }, // 5:00 min
    hell:   { label: 'Inferno', cols: 42, rows: 28, cell: 32, time: 180, color: '#ff0000', wallColor: '#ff4d4d', itemCount: 28, itemValue: 28 }
  };
  ...
  const state = {
    ...
    path: [],
    collectibles: [],
    player: { x: 0, y: 0, px: 0, py: 0, size: 0.55, speed: 1.0 }, // Velocidade reduzida para 1.8
    inputs: { up: 0, down: 0, left: 0, right: 0 },
    infinite: false,
    infiniteLevel: 1,
    highScore: Number(localStorage.getItem('datamaze_highscore')||0),
    rain: { cols: [], fontSize: 16 },
    lastTime: 0, // Adicionado para controle de tempo consistente
    accumulatedTime: 0 // Adicionado para controle de tempo consistente
  };
  ...
  function startRun() {
    // Se for campanha, definir a dificuldade correta baseada no índice
    if (state.campaign) {
      const campaignOrder = state.campaign50 ? state.campaign50Order : ['easy', 'medium', 'hard'];
      const currentDiff = campaignOrder[state.campaignIndex];
      setDifficulty(currentDiff);
      
      const maxLevels = state.campaign50 ? 50 : 3;
      levelLabel.textContent = `Campanha ${state.campaign50 ? '50' : '3'} Fases - ${state.campaignIndex + 1}/${maxLevels}`;
    }
    ...
    state.player.speed = 1.8; // Velocidade consistente
  ...
  function stepPlayer(dt) {
    ...
    const cell = diff.cell;
    // Usar um valor fixo para a velocidade, independente do dt
    const vel = state.player.speed;
  ...
  function update(timestamp) {
    ...
    // Cálculo consistente do delta time
    if (!state.lastTime) state.lastTime = timestamp;
    const deltaTime = timestamp - state.lastTime;
    state.lastTime = timestamp;
    
    // Passar um valor fixo para stepPlayer para movimento consistente
    stepPlayer(16.666); // 60 FPS
    
    // Atualizar o tempo de forma consistente
    ...
    if ((px === state.goal.x && py === state.goal.y) || 
      ...
      // Adicionar bônus de tempo se houver tempo restante
      const timeBonus = Math.floor(state.timeLeft * 2);
      if (timeBonus > 0) {
        state.phaseScore += timeBonus;
      }
      ...
      if (state.campaign) {
        ...
        if (state.campaignIndex >= (state.campaign50 ? 49 : 2)) {
  ...
  function endRun(success) {
    ...
    if (success) {
      // Adicionar pontos de bônus por tempo restante
      state.phaseScore += timeBonus;
      state.score += state.phaseScore;
      
      // Garantir que a velocidade permaneça consistente
      state.player.speed = 1.8;
      
      // Verificar se é campanha e se completou todos os níveis
      if (state.campaign) {
        const campaignOrder = state.campaign50 ? state.campaign50Order : state.campaignOrder;
        const nextIndex = state.campaignIndex + 1;
        const maxLevels = state.campaign50 ? 50 : 3;
        
        if (nextIndex < maxLevels) {
          // Próxima fase
          state.campaignIndex = nextIndex;
          const nextDiff = campaignOrder[nextIndex];
          setDifficulty(nextDiff);
          
          // Mostrar mensagem de fase concluída
          endTitle.textContent = `Fase ${nextIndex}/${maxLevels} Concluída!`;
          endSubtitle.textContent = `Próxima fase: ${DIFFS[nextDiff].label}`;
          finalScore.textContent = state.score;
          finalTime.textContent = formatTime(state.totalTimeAccum + (diff.time - state.timeLeft));
          
          // Atualizar o botão para próxima fase
          btnRetry.textContent = `Próxima Fase (${nextIndex + 1}/${maxLevels})`;
          btnRetry.onclick = () => {
            endScreen.classList.remove('visible');
            startRun();
          };
          
          // Mostrar tela de fim de fase
          endScreen.classList.add('visible');
          return;
        } else {
          // Verificar se atingiu a pontuação mínima para vencer
          if (state.score >= 750) {
            // Campanha concluída com sucesso
            endTitle.textContent = 'Campanha Concluída!';
            endSubtitle.textContent = 'Você completou todas as fases e atingiu a pontuação necessária!';
          } else {
            // Não atingiu a pontuação mínima
            endTitle.textContent = 'Falha na Missão';
            endSubtitle.textContent = `Você completou as fases, mas precisa de 750 pontos para vencer! (${state.score}/750)`;
            success = false; // Marca como falha para não adicionar pontos extras
          }
        }
      } else {
        // Modo normal - vitória
        endTitle.textContent = 'Transferência concluída';
        endSubtitle.textContent = 'Dados entregues com sucesso!';
      }
    } else {
      // Derrota
      endTitle.textContent = 'Conexão perdida';
      endSubtitle.textContent = 'Tempo esgotado ou pacote corrompido';
    }
    ...
    // Configurar botões finais
    btnRetry.textContent = 'Jogar Novamente';
    btnRetry.onclick = () => {
      endScreen.classList.remove('visible');
      startRun();
    };
    ...
    // Salvar pontuação máxima se for o caso
    if (state.score > state.highScore) {
      state.highScore = state.score;
      localStorage.setItem('datamaze_highscore', state.highScore);
    }
  }
  ...
  // Gerar ordem da campanha de 50 fases
  function generateCampaign50Order() {
    const order = [];
    // Primeiras 15 fases: fácil (verde)
    for (let i = 0; i < 15; i++) {
      order.push('easy');
    }
    // Próximas 15 fases: médio (amarelo)
    for (let i = 0; i < 15; i++) {
      order.push('medium');
    }
    // Próximas 15 fases: difícil (laranja/vermelho)
    for (let i = 0; i < 15; i++) {
      order.push('hard');
    }
    // Últimas 5 fases: inferno (vermelho intenso)
    for (let i = 0; i < 5; i++) {
      order.push('hell');
    }
    return order;
  ...
  // Inicializar a ordem da campanha de 50 fases
  state.campaign50Order = generateCampaign50Order();

  // Configurar botão de nova página
  if (btnNewPage) {
  ...
  // UI wiring - Single difficulty buttons
  ...
  // Campanha de 50 fases
  if (btnCampaign50) {
    btnCampaign50.addEventListener('click', (e) => {
      // Iniciar a campanha de 50 fases
      state.campaign = true;
      state.campaign50 = true;
      state.infinite = false;
      state.campaignIndex = 0;
      state.score = 0;
      state.totalTimeAccum = 0;
      
      // Usar a primeira dificuldade da campanha de 50 fases
      const firstDiff = state.campaign50Order[0];
  ...
  if (btnCampaign) {
    btnCampaign.addEventListener('click', () => {
      ...
      state.infinite = false;
      state.campaignIndex = 0;
      state.score = 0;  // Zerar a pontuação ao iniciar nova campanha
      state.difficulty = 'easy';  // Definir dificuldade inicial como fácil
      state.totalTimeAccum = 0;
      state.score = 0;
      startRun();
    });
